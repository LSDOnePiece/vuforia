...
-(Vuforia::Matrix44F) getVideoBackgroundProjMatrix
{
    return Vuforia::Tool::convert2GLMatrix( self.currentRenderingPrimitives->getVideoBackgroundProjectionMatrix(self.currentView, Vuforia::COORDINATE_SYSTEM_CAMERA));
}
...
- (void)renderFrameVuforia
{
    ...
    for (int viewIdx = 0; viewIdx < viewList.getNumViews(); viewIdx++) {
        ...
        if (self.currentView != Vuforia::VIEW_POSTPROCESS) {
            [self.control renderFrameWithState:state projectMatrix:projectionMatrix andViewport: viewport];
        }
