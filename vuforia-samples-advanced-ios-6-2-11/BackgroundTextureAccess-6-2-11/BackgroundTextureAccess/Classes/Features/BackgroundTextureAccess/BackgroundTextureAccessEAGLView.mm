...
@synthesize touchLocation_X, touchLocation_Y, vapp;
...
- (void)renderFrameWithState:(const Vuforia::State &)state projectMatrix:(Vuforia::Matrix44F &)projectionMatrix
{
    ...
    glUniformMatrix4fv(videoBackgroundShader.vbProjectionMatrixHandle, 1, GL_FALSE, &vbProjectionMatrix.data[0]);
    ...
    for (int tIdx = 0; tIdx < state.getNumTrackableResults(); tIdx++)
    {
        ...
        Vuforia::Tool::convertPose2GLMatrix(trackableResult->getPose());
    ...
    ////////////////////////////////////////////////////////////////////////////
    // It is always important to tell the Vuforia Renderer that we are finished
...
- (void)CreateVideoBackgroundMesh
{
    ...
    const Vuforia::VideoBackgroundTextureInfo texInfo=Vuforia::Renderer::getInstance().getVideoBackgroundTextureInfo();
    ...
    const Vuforia::VIDEO_BACKGROUND_REFLECTION reflection = Vuforia::Renderer::getInstance().getVideoBackgroundConfig().mReflection;
    ...
    for (int j=vbNumVertexCols - 1; j>=0; j--)
    {
        for (int i=0; i<vbNumVertexRows; i++)
        {
            // We also populate its associated texture coordinate
            vbOrthoQuadTexCoords[currentCoordPosition   /*U*/] = ((reflection == Vuforia::VIDEO_BACKGROUND_REFLECTION_ON) ? uSlope * (reflectionOffset - j) : uSlope * j);
            vbOrthoQuadTexCoords[currentCoordPosition+1 /*V*/] = vRatio - (vSlope*i);
