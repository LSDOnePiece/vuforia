...
ButtonOverlayViewController* buttonOverlay;
NSArray* textures;
...
int dropStartIndex;
Vuforia::Vec2F lastDropPosition(0, 0);

Domino* selectedDomino;
int selectedDominoIndex;

Vuforia::Vec3F dominoBaseVertices[8];
Vuforia::Vec3F dominoTransformedVerticesA[8];
Vuforia::Vec3F dominoTransformedVerticesB[8];
Vuforia::Vec3F dominoNormals[3];
...
void dominoesSetButtonOverlay(ButtonOverlayViewController* overlay) { buttonOverlay = overlay; }
...
void dominoesSetTexSampler2DHandle(int t2dh) { texSampler2DHandle = t2dh; }
...
void
renderDominoes(const Vuforia::State& state, Vuforia::Matrix44F& projectionMatrix)
{
    ...
    if (state.getNumTrackableResults() > 0) {
        ...
        assert(trackableResult->isOfType(Vuforia::ImageTargetResult::getClassType()));
        ...
        const Vuforia::VirtualButtonResult* button = imageTargetResult->getVirtualButtonResult("startButton");
...
void
updateAugmentation(const Vuforia::Trackable* trackable, float dt)
{
...
handleTouches()
{
    ...
    if (touch1.didTap && touch1.startTime > lastTapTime) {
        ...
        if (selected == NULL && selectedDomino == NULL) {
            ...
            Vuforia::Vec2F position(intersection.data[0], intersection.data[1]);
        ...
        if (touch1.dt > MAX_TAP_TIMER || touch1.dist2 > MAX_TAP_DISTANCE2) {
            ...
            if (selectedDomino != NULL) {
                ...
                Vuforia::Vec3F intersection, lineStart, lineEnd;
                projectScreenPointToPlane(Vuforia::Vec2F(touch1.x, touch1.y), Vuforia::Vec3F(0, 0, 0), Vuforia::Vec3F(0, 0, 1), intersection, lineStart, lineEnd);
...
void
renderAugmentation(const Vuforia::Trackable* trackable)
{
...
renderCube(float* transform)
{
    ...
    ShaderUtils::multiplyMatrix(&currentProjectionMatrix.data[0], &objectMatrix.data[0], &modelViewProjection.data[0]);
...
void
virtualButtonOnUpdate(Vuforia::State& state)
{
    if (shouldUpdateButton)
    {
        ...
        Vuforia::ImageTarget* target = [vUtils getImageTarget:0];
        
        [vUtils allowDataSetModification];
        
        // Get the virtual button we created earlier, if it exists
        Vuforia::VirtualButton* button = target->getVirtualButton("startButton");
        
        if (shouldAddButton && button == NULL)
        {
            Vuforia::Rectangle vbRectangle(0, 0, 0, 0);
        ...
        [vUtils saveDataSetModifications];
        shouldUpdateButton = false;
    }
}
...
initDominoBaseVertices()
{
    ...
    dominoBaseVertices[0] = Vuforia::Vec3F(1.0f, 1.0f, 1.0f);
    dominoBaseVertices[1] = Vuforia::Vec3F(-1.0f, 1.0f, 1.0f);
    dominoBaseVertices[2] = Vuforia::Vec3F(1.0f, -1.0f, 1.0f);
    dominoBaseVertices[3] = Vuforia::Vec3F(-1.0f, -1.0f, 1.0f);
    dominoBaseVertices[4] = Vuforia::Vec3F(1.0f, 1.0f, -1.0f);
    dominoBaseVertices[5] = Vuforia::Vec3F(-1.0f, 1.0f, -1.0f);
    dominoBaseVertices[6] = Vuforia::Vec3F(1.0f, -1.0f, -1.0f);
    dominoBaseVertices[7] = Vuforia::Vec3F(-1.0f, -1.0f, -1.0f);
}
...
initDominoNormals()
{
    ...
    dominoNormals[0] = Vuforia::Vec3F(1, 0, 0);
    dominoNormals[1] = Vuforia::Vec3F(0, 1, 0);
    dominoNormals[2] = Vuforia::Vec3F(0, 0, 1);
}
...
bool
canDropDomino(Vuforia::Vec2F position)
{
    Domino* domino;
    bool isClear = false;
...
void
dropDomino(Vuforia::Vec2F position)
{
    ...
    float angle = 0.0f;
    if (dominoCount > 0) {
        Vuforia::Vec2F d = SampleMath::Vec2FSub(position, lastDropPosition);
...
bool
checkIntersection(Vuforia::Matrix44F transformA, Vuforia::Matrix44F transformB)
{
    ...
    Vuforia::Vec3F normalA1 = SampleMath::Vec3FTransformNormal(dominoNormals[0], transformA);
    Vuforia::Vec3F normalA2 = SampleMath::Vec3FTransformNormal(dominoNormals[1], transformA);
    Vuforia::Vec3F normalA3 = SampleMath::Vec3FTransformNormal(dominoNormals[2], transformA);
    
    Vuforia::Vec3F normalB1 = SampleMath::Vec3FTransformNormal(dominoNormals[0], transformB);
    Vuforia::Vec3F normalB2 = SampleMath::Vec3FTransformNormal(dominoNormals[1], transformB);
    Vuforia::Vec3F normalB3 = SampleMath::Vec3FTransformNormal(dominoNormals[2], transformB);
    
    Vuforia::Vec3F edgeAxisA1B1 = SampleMath::Vec3FCross(normalA1, normalB1);
    Vuforia::Vec3F edgeAxisA1B2 = SampleMath::Vec3FCross(normalA1, normalB2);
    Vuforia::Vec3F edgeAxisA1B3 = SampleMath::Vec3FCross(normalA1, normalB3);
    
    Vuforia::Vec3F edgeAxisA2B1 = SampleMath::Vec3FCross(normalA2, normalB1);
    Vuforia::Vec3F edgeAxisA2B2 = SampleMath::Vec3FCross(normalA2, normalB2);
    Vuforia::Vec3F edgeAxisA2B3 = SampleMath::Vec3FCross(normalA2, normalB3);
    
    Vuforia::Vec3F edgeAxisA3B1 = SampleMath::Vec3FCross(normalA3, normalB1);
    Vuforia::Vec3F edgeAxisA3B2 = SampleMath::Vec3FCross(normalA3, normalB2);
    Vuforia::Vec3F edgeAxisA3B3 = SampleMath::Vec3FCross(normalA3, normalB3);
...
bool
isSeparatingAxis(Vuforia::Vec3F axis)
{
