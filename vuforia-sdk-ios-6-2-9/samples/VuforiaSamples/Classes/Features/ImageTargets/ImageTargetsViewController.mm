...
- (bool) doInitTrackers {
    #pragma mark -- 初始物体跟踪器 Vuforia::Tracker initTracker
...
- (bool) doLoadTrackersData {
    ...
    #pragma mark -- 加载跟踪数据(通过vuforia官方生成的数据库) 这里设置了2个（dataSetStonesAndChips和dataSetTarmac）
    dataSetMyDevice = [self loadObjectTrackerDataSet:@"MyDevice.xml"];
    ...
    if ((dataSetStonesAndChips == NULL) || (dataSetTarmac == NULL) || (dataSetMyDevice == NULL)) {
        NSLog(@"Failed to load datasets");
        return NO;
    }
    
    if (! [self activateDataSet:dataSetMyDevice]) {
...
- (void) onInitARDone:(NSError *)initError {
    ...
    #pragma mark -- 初始化AR后的回调
    UIActivityIndicatorView *loadingIndicator = (UIActivityIndicatorView *)[eaglView viewWithTag:1];
    [loadingIndicator removeFromSuperview];
    
    if (initError == nil) {
        NSError * error = nil;
        #pragma mark -- 开始AR 初始化摄像头
...
- (void) onVuforiaUpdate: (Vuforia::State *) state
{
    ...
    if (switchToMyDevice) {
        [self activateDataSet:dataSetMyDevice];
        switchToMyDevice = NO;
    }
}
...
- (bool) doUnloadTrackersData {
    ...
    if (!objectTracker->destroyDataSet(dataSetMyDevice))
    {
        NSLog(@"Failed to destroy data set dataSetMyDevice.");
    }
...
- (BOOL)activateDataSet:(Vuforia::DataSet *)theDataSet
{
    ...
    #pragma mark -- 激活并设置当期Vuforia::DataSet*  dataSetCurrent
    ...
    else
    {
        ...
        else
        {
            ...
            if (dataSetCurrent == dataSetMyDevice) {
                NSLog(@"myDevice");
            }
...
- (BOOL) menuProcess:(NSString *)itemName value:(BOOL)value
{
    ...
    if ([@"Flash" isEqualToString:itemName]) {
        ...
        switchToMyDevice = NO;
        return true;
    }else if ([@"MyDevice" isEqualToString:itemName])
    {
        [self setExtendedTrackingForDataSet:dataSetCurrent start:NO];
        switchToTarmac = NO;
        switchToStonesAndChips = NO;
        switchToMyDevice = YES;
        return true;
    }
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue isKindOfClass:[PresentMenuSegue class]]) {
        ...
        if ([dest isKindOfClass:[SampleAppMenuViewController class]]) {
            ...
            NSLog(@"Dataset current %@", dataSetCurrent == dataSetMyDevice ? @"MyDevice" : @"Stones & Chips");
            ...
            if (dataSetCurrent == dataSetStonesAndChips) {
                ...
                [menuVC setValue:NO forMenuItem:@"MyDevice"];
            }
            else if (dataSetCurrent == dataSetTarmac){
                [menuVC setValue:YES forMenuItem:@"Tarmac"];
                [menuVC setValue:NO forMenuItem:@"Stones & Chips"];
                [menuVC setValue:NO forMenuItem:@"MyDevice"];
            }
            else if (dataSetCurrent == dataSetMyDevice){
                [menuVC setValue:NO forMenuItem:@"Tarmac"];
                [menuVC setValue:NO forMenuItem:@"Stones & Chips"];
                [menuVC setValue:YES forMenuItem:@"MyDevice"];
