...
- (id)initWithFrame:(CGRect)frame appSession:(SampleApplicationSession *) app
{
    ...
    if (self) {
        ...
        objects3D = [NSMutableArray array];
...
- (void) renderFrameWithState:(const Vuforia::State&) state projectMatrix:(Vuforia::Matrix44F&) projectionMatrix {
    ...
    for (int i = 0; i < state.getNumTrackableResults(); ++i) {
        ...
        else if (!strcmp(trackable.getName(), "shibietu")){
            targetIndex = 0;
            
            Object3D *obj3D = [objects3D objectAtIndex:targetIndex];
            glDrawArrays(GL_TRIANGLES, 0, obj3D.numIndices);
        }
...
- (void) setup3dObjects
{
    ...
    obj3D.numVertices = planeNumVerts;
    obj3D.vertices = planeVerts;
    obj3D.normals = planeNormals;
    obj3D.texCoords = planeTexCoords;
    
    obj3D.numIndices = planeNumVerts;
    //  obj3D.indices = bananaVerts;
    
    obj3D.texture = augmentationTexture[0];
