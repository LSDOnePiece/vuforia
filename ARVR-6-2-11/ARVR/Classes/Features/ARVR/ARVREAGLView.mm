...
- (void) renderARWorldWithTargetPose:(float *)targetPose
                    projectionMatrix:(float *)projectionMatrix
                              viewId:(Vuforia::VIEW)viewId
                            viewport:(Vuforia::Vec4I)viewport
                   isARObjectVisible:(bool) isARObjectVisible;
@end
...
- (void)renderFrameVuforia
{
    ...
    for (int viewIdx = 0; viewIdx < viewList.getNumViews(); viewIdx++) {
        ...
        if (mIsVR) {
            ...
            [self renderARWorldWithTargetPose:&targetPose.data[0]
                             projectionMatrix:&projectionMatrix.data[0]
                                       viewId:vw
                                     viewport:viewport
                            isARObjectVisible:isARObjectVisible];
        }
                   ...
                   isARObjectVisible:(bool)arObjectVisible
{
    ...
    if(Vuforia::Device::getInstance().isViewerActive()) {
        float sceneScaleFactor = [self getSceneScaleFactorWithViewId:viewId];        
        projectionMatrix[0] *= sceneScaleFactor;
        projectionMatrix[5] *= sceneScaleFactor;
    }
    ...
    if (arObjectVisible)
    {
        ...
        SampleApplicationUtils::multiplyMatrix(projectionMatrix, targetPose, &modelViewProjection.data[0]);
